...
\begin{frame}{Decentralized Optimal Control in Large Exchangeable Teams}
        \uncover<+->{\begin{alertblock}{\textbf{Theorem 10} [Sanjari et al. (2024)]}
        ...
        \end{align*}}
        \uncover<+->{where $\Gamma$ is the set of admissable team policies, and $\gamma^{N,\ast}=(\gamma^{N,\ast}_t)_{t=0}^{T-1}$ is randomized, decentralized, and symmetric in the sense that}
        \uncover<+->{
            \begin{align*}
    ...
    \end{alertblock}}
    \uncover<+->{}
    \uncover<+->{Could be of tremendous value in theoretical neuroscience, but at what spatial scale might we expect this to be hold?}
\end{frame}
